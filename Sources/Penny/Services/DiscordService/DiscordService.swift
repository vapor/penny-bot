...
actor DiscordService {
    ...
    private func getDMChannelId(userId: UserSnowflake) async -> ChannelSnowflake? {
        if let existing = dmChannels[userId] {
            ...
            do {
                let dmChannel =
                    try await discordClient.createDm(
    ...
    func getPossiblyCachedChannelMessage(
        ...
        guard let cached = self.cachedMessages[[AnySnowflake(channelId), AnySnowflake(messageId)]]
        else {
            guard let message = await getChannelMessage(channelId: channelId, messageId: messageId)
            else {
                return nil
            }
    ...
    func getChannelMessage(
        ...
        do {
            return
                try await discordClient.getMessage(
                    channelId: channelId,
                    messageId: messageId
                )
                .decode()
