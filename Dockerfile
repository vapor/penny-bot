...
# Build the executable, with optimizations, with static linking, and using jemalloc
...
RUN cp "$(swift build --package-path /build -c $SWIFT_CONFIGURATION --show-bin-path)/$EXEC_NAME" ./
