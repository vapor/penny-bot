...
class GatewayProcessingTests: XCTestCase {
    ...
    func testAutoPings() async throws {
        ...
        do {
            let payload: Never? = try XCTUnwrap(createDM as? Never?)
    ...
    func testHowManyCoins() async throws {
        do {
            ...
            XCTAssertEqual(
                message,
                "<@290483761559240704> has 2591 \(Constants.ServerEmojis.coin.emoji)!"
            )
        }
        ...
        do {
            ...
            XCTAssertEqual(
                message,
                "<@961607141037326386> has 2591 \(Constants.ServerEmojis.coin.emoji)!"
            )
        }
    }
    ...
    func testProposalsChecker() async throws {
        ...
        do {
            ...
            XCTAssertEqual(
                embed.title,
                "[SE-0001] In Active Review: Allow (most) keywords as argument labels"
            )
            XCTAssertEqual(
                embed.description,
    ...
    func testFaqsCommand() async throws {
        ...
        do {
            ...
            let message = try XCTUnwrap(response.embeds?.first?.description)
            XCTAssertTrue(
                message.hasPrefix("You don't have access to this command; it is only available to"),
                message
            )
        }
    ...
    func testAutoFaqsCommand() async throws {
        ...
        do {
            ...
            let response =
                ...
                .value
            let payload: Never? = try XCTUnwrap(response as? Never?)
            XCTAssertEqual(payload, .none)
        }
    }
}
